//<<<<<<<<<<<<<<<<<<<<COMPOSITION DESIGNPATTERN>>>>>>>>>>>>>>>>>>>>>>>>>
// enables to use  at the same time multiple objects that share the same API
// When we need to control multiple objects at the same time we can use a composite to do this (eg: $ in JQuery to select multiple elements)
// This ideal in recursive elements that are "chained" to each other
(function(win, $) {
